...
class Migration_Add_whatsapp_message_logs_indexes extends EA_Migration
{
    ...
    public function up()
    {
        ...
        if (!$this->db->query("SHOW INDEX FROM whatsapp_message_logs WHERE Key_name = 'idx_body_hash'")->num_rows()) {
            $this->db->query('ALTER TABLE whatsapp_message_logs ADD INDEX idx_body_hash (body_hash)');
        }
        ...
        if (!$this->db->query("SHOW INDEX FROM whatsapp_message_logs WHERE Key_name = 'idx_duplicate_check'")->num_rows()) {
            $this->db->query('
        ...
        if (!$this->db->query("SHOW INDEX FROM whatsapp_message_logs WHERE Key_name = 'idx_appointment_datetime'")->num_rows()) {
            $this->db->query('
                ALTER TABLE whatsapp_message_logs
        ...
        if (!$this->db->query("SHOW INDEX FROM whatsapp_message_logs WHERE Key_name = 'idx_result_datetime'")->num_rows()) {
            $this->db->query('
                ALTER TABLE whatsapp_message_logs
        ...
        if ($this->db->table_exists('whatsapp_routine_sends')) {
            if (!$this->db->query("SHOW INDEX FROM whatsapp_routine_sends WHERE Key_name = 'idx_routine_appointment'")->num_rows()) {
                $this->db->query('
                    ALTER TABLE whatsapp_routine_sends
    ...
    public function down()
    {
        ...
        foreach ($indexes_to_drop as $index_name) {
            if ($this->db->query("SHOW INDEX FROM whatsapp_message_logs WHERE Key_name = '{$index_name}'")->num_rows()) {
                $this->db->query("ALTER TABLE whatsapp_message_logs DROP INDEX {$index_name}");
            }
        }
        ...
        if ($this->db->table_exists('whatsapp_routine_sends')) {
            if ($this->db->query("SHOW INDEX FROM whatsapp_routine_sends WHERE Key_name = 'idx_routine_appointment'")->num_rows()) {
                $this->db->query('ALTER TABLE whatsapp_routine_sends DROP INDEX idx_routine_appointment');
            }
        }
    }
}

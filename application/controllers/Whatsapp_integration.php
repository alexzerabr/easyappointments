...
class Whatsapp_integration extends EA_Controller
{
    ...
    public function save(): void
    {
        try {
            ...
            // Update with new values (normalize: host may carry port; ignore legacy port field)
            foreach ($whatsapp_settings as $setting) {
                ...
                if ($name === 'host') {
                    $settings_data['host'] = trim((string)$value);
                } elseif ($name === 'port') {
            ...
            if ($all_filled) {
                try {
                    // Update service config in-memory so subsequent calls use provided host (with optional port) and session
        ...
        try {
            ...
            $this->whatsapp_message_logs_model->update_status(
                ...
                [
                    ...
                    'update_datetime' => date('Y-m-d H:i:s'),
                    'error_message' => $isSuccess ? null : ($response['message'] ?? 'Falha ao enviar mensagem')
                ]
            );
            ...
            try {
                if (isset($hash)) {
                    $this->whatsapp_message_logs_model->update_status($hash, 'FAILURE', [
                        'update_datetime' => date('Y-m-d H:i:s'),

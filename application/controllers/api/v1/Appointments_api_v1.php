...
class Appointments_api_v1 extends EA_Controller
{
    ...
    public function __construct()
    {
        ...
        $this->load->library('websocket_broadcaster');
    ...
    private function aggregates(array &$appointment): void
    {
        ...
        if ($aggregates) {
            log_message('error', 'API DEPRECATION WARNING: The "aggregates" parameter is deprecated and will be removed in future versions. Use "attach" instead.');
    ...
    private function notify_and_sync_appointment(array $appointment, string $action = 'store'): void
    {
        ...
        if ($action === 'store') {
            $this->websocket_broadcaster->appointment_created($appointment);
        } else {
            $this->websocket_broadcaster->appointment_updated($appointment);
        }
    }
    ...
    public function destroy(int $id): void
    {
        try {
            ...
            // Broadcast WebSocket event for real-time updates
            $this->websocket_broadcaster->appointment_deleted($deleted_appointment);

...
class Wppconnect_service
{
    ...
    private function load_config(): void
    {
        ...
        if (empty($settings)) {
            $this->config = [
                'host' => getenv('WPP_HOST') ?: 'http://localhost:21465',
                // Port deprecated; keep key for backward compatibility when present
                'port' => getenv('WPP_PORT') ?: null,
        ...
        $this->config = [
            'host' => $settings['host'] ?? 'http://localhost:21465',
            // keep reading port if present in legacy data
    ...
    {
        // Port no longer required; host may include port
        $required_fields = ['host', 'session'];
        $missing = [];
    ...
    public function is_configured(): bool
    {
        ...
        // Port field no longer required; host may embed port
        return !empty($host) && !empty($session);
    }
    ...
    public function get_config(): array
    {
        return [
            'host' => $this->config['host'],
            // port intentionally omitted from public config snapshot
            'session' => $this->config['session'],

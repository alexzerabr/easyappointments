...
class Api
{
    ...
    /**
     * Authenticated JWT user data.
     *
     * @var object|null
     */
    protected ?object $jwt_user = null;
    ...
    /**
     * Authorize the API request (JWT, Bearer Token, or Basic Auth supported).
     *
     * Authentication methods are checked in this order:
     * 1. JWT Bearer token (new)
     * 2. API token (global Bearer token)
    ...
    public function auth(): void
    {
        try {
            ...
            if (!empty($bearer_token)) {
                ...
                if ($this->validate_jwt($bearer_token)) {
                    return;
                }

                // 2. Try global API token
                $api_token = setting('api_token');

                if (!empty($api_token) && $api_token === $bearer_token) {
                    return;
                }
            }
            ...
            throw new RuntimeException('No valid authentication provided');
        } catch (ExpiredException $e) {
            // JWT token expired - return specific error
            $this->request_jwt_expired();
        } catch (Throwable) {
     ...
     */
    protected function validate_jwt(string $token): bool
    {
        try {
            $this->CI->load->library('jwt_auth');
            $decoded = $this->CI->jwt_auth->validate_access_token($token);

            // Store user data for later use
            $this->jwt_user = $decoded->data;

            return true;
        } catch (ExpiredException $e) {
            // Re-throw to handle specifically
            throw $e;
        } catch (Throwable $e) {
            // Invalid JWT - not a JWT token, continue to next auth method
            return false;
        }
    }

    /**
     * Get the authenticated JWT user data.
     *
     * @return object|null Returns the JWT user data or null if not authenticated via JWT.
     */
    public function get_jwt_user(): ?object
    {
        return $this->jwt_user;
    }

    /**
     * Returns JWT expired response.
     */
    #[NoReturn]
    public function request_jwt_expired(): void
    {

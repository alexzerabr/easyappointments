...
if (empty($env_key)) {
    // Fallback to legacy default but log warning (Security Fix)
    log_message('error', 'SECURITY WARNING: Using insecure default encryption key. Please set ENCRYPTION_KEY environment variable.');
...
// Session Path Detection (Docker vs Manual)
if (getenv('IS_DOCKER') || file_exists('/.dockerenv')) {
    $config['sess_save_path'] = '/var/www/html/storage/sessions';
} else {
